...
export class AmazonPricing implements PricingPlan {
  calculateTotal(items: Item[]) {
    const discountedPrices = items.map(item => item.name === 'Large Pizza' ? ({ ...item, price: 29999 }): item)
    return discountedPrices.reduce((total,current) => total+(current.price/100),0) 
  }
}
...
export class FacebookPricing implements PricingPlan {
  calculateTotal(items: Item[]) {
    ...
    const discountedPrices = items.map(item => item.name === 'Large Pizza' ? { ...item, price: 38999 } : item)
    
    const subtotal = discountedPrices.reduce((total, current) => total + (current.price), 0) 
    return (subtotal - discount)/100
  }
}